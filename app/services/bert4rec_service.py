...
from models.bert4rec import bert4rec_model

class Bert4RecService(BaseRecommendationService):
    def __init__(self, bert4rec_checkpoint, user_id_to_index, item_id_to_index, item_index_to_id):
        self.bert4rec_checkpoint = bert4rec_checkpoint
        self.user_id_to_index = user_id_to_index
        self.item_id_to_index = item_id_to_index
        self.item_index_to_id = item_index_to_id
        self.max_seq_len = int(self.bert4rec_checkpoint["max_seq_len"])
        self.pad_id = int(self.bert4rec_checkpoint["pad_id"])
        self.mask_id = int(self.bert4rec_checkpoint["mask_id"])

        checkpoint_state = self.bert4rec_checkpoint["model_state_dict"]

        self.model = bert4rec_model.BERT4RecModel(
            num_items= int(self.bert4rec_checkpoint["num_items"]),
            max_seq_len=int(self.bert4rec_checkpoint["max_seq_len"]),
            embedding_dim=int(self.bert4rec_checkpoint["embedding_dim"]),
            num_attention_heads=int(self.bert4rec_checkpoint["num_attention_heads"]),
            num_transformer_blocks=int(self.bert4rec_checkpoint["num_transformer_blocks"]),
            dropout_rate=float(self.bert4rec_checkpoint["dropout_rate"]),
        )
    ...
    def build_interactions_sequence(self, interactions):
        ...
        #Add mask token at the end of the sequence for prediction
        user_sequence.append(self.mask_id)
